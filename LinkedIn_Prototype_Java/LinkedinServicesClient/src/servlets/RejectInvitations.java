...
/**
 * Servlet implementation class RejectInvitations
 */
@WebServlet("/rejectInvitation")
public class RejectInvitations extends HttpServlet {
	private static final long serialVersionUID = 1L;
	ServiceProxy proxy = new ServiceProxy();
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RejectInvitations() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("Session : "+session);
		 ...
		 try
		 {
			 ...
			 rejectInvitations= proxy.rejectInvitations(connectionID);
			 System.out.println("Connection is accepted successfully");
			 
			 request.getRequestDispatcher("rejectInvitation.jsp").forward(request, response);
		 }
		 catch(Exception e)
		 {
			e.printStackTrace();
